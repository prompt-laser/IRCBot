...
PORT = 6667
BUFFER_SIZE = 1024
COMMAND_CHAR = command_char_string				#"@"
CHANNEL = channel_name_string					#"myChannel"
USER = username_string							#"myUsername"
NICK = nick_string								#"myNick"
REAL_NAME = real_name_string					#"Mr. Smith"
						...
						for mod in modList:
							if(mod.trigger == command.lower()):
								if(arguments == "help"):
									...
									pluginResponse = call_plugin(mod.name, arguments)
									if(pluginResponse.split(" ")[0].lower() != "error"):
										try:
											s.send(bytes("PRIVMSG " + CHANNEL + " :" + pluginResponse + "\r\n", "ASCII"))
										except Exception as e:
											s.send(bytes("PRIVMSG " + CHANNEL + " :" + call_plugin("error", "insert Error getting bytes from string from command " + mod.name + ": " + pluginResponse) + "\r\n", "ASCII"))
									else:
										s.send(bytes("PRIVMSG " + CHANNEL + " :" + call_plugin("error", "insert " + pluginResponse) + "\r\n", "ASCII"))
